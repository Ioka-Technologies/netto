    ...
    } in stack
    {
        drawing_area.draw(&Rectangle::new(
            [
                ...
                (
                    position.0 + BAR_WIDTH,
                    position.1 + height * (acc_fraction + fraction),
                ),
            ],
            color.filled(),
        ))?;

        let section_height_px = (drawing_area.as_coord_spec().translate(&(0.0f32, 0.0f32)).1
            - drawing_area
                .as_coord_spec()
                .translate(&(0.0f32, fraction))
                .1)
            .unsigned_abs();
        ...
        if section_height_px > *TEXT_HEIGHT.get().unwrap() {
            drawing_area.draw(&Text::new(
                name,
                (
                    position.0 + BAR_WIDTH / 2.0,
                    position.1 + height * (acc_fraction + fraction / 2.0),
                ),
...
{
    let kernel =
        (metrics.procfs_metrics[2] + metrics.procfs_metrics[5] + metrics.procfs_metrics[6])
            / metrics.num_possible_cpus as f64;
    let user =
        (metrics.procfs_metrics[0] + metrics.procfs_metrics[1]) / metrics.num_possible_cpus as f64;
    ...
    <DB as DrawingBackend>::ErrorType: 'static,
{
    ...
    let stack = metrics
        ...
        .map(|m| {
            (
                m.name.as_str(),
                m.cpu_fracs.iter().sum::<f64>()
                    / (metrics.num_possible_cpus as f64 * kernel_adjusted),
            )
        })
        .chain(std::iter::once((
            "other",
            (kernel_adjusted - networking) / kernel_adjusted,
        )))
        .zip(colors)
    ...
    drawing_area.draw(&PathElement::new(
        [
            (
                0.16 + BAR_WIDTH / 2.0,
                0.5 + (kernel as f32 - 0.5) * BAR_HEIGHT,
            ),
            (0.5 - BAR_WIDTH / 2.0, 0.5 + BAR_HEIGHT / 2.0),
        ],
        BLACK,
    ...
    <DB as DrawingBackend>::ErrorType: 'static,
{
    ...
    let sub_metrics = rx_softirq_metric.sub_metrics.iter().flat_map(|s| {
        if s.cpu_fracs.len() == metrics.num_possible_cpus {
            vec![(
                s.name.clone(),
                s.cpu_fracs.iter().sum::<f64>() / metrics.num_possible_cpus as f64,
            )]
        } else {
            s.sub_metrics
                .iter()
                .map(|ss| {
                    (
                        format!("{}/{}", s.name, ss.name),
                        ss.cpu_fracs.iter().sum::<f64>() / metrics.num_possible_cpus as f64,
                    )
                })
                .collect()
        }
    ...
    let total_calc = sub_metrics.clone().map(|(_, f)| f).sum::<f64>();
    let total_meas =
        rx_softirq_metric.cpu_fracs.iter().sum::<f64>() / metrics.num_possible_cpus as f64;
    let total = total_meas.max(total_calc);
    let other = total - total_calc;

    let stack = sub_metrics
        .chain(std::iter::once(("other".to_string(), other)))
        .zip(colors)
        .map(|((name, fraction), (color, text_color))| BoxSpec {
            name,
            fraction: (fraction / total) as _,
            color,
            text_color,
        });
    ...
    drawing_area.draw(&PathElement::new(
        [
            (
                0.5 + BAR_WIDTH / 2.0,
                0.5 + (((rx_softirq_height + total_meas) / kernel_adjusted) as f32 - 0.5)
                    * BAR_HEIGHT,
            ),
            (0.83 - BAR_WIDTH / 2.0, 0.5 + BAR_HEIGHT / 2.0),
        ],
    ...
    drawing_area.draw(&PathElement::new(
        [
            (
                0.5 + BAR_WIDTH / 2.0,
                0.5 + ((rx_softirq_height / kernel_adjusted) as f32 - 0.5) * BAR_HEIGHT,
            ),
            (0.83 - BAR_WIDTH / 2.0, 0.5 - BAR_HEIGHT / 2.0),
        ],
        BLACK,
