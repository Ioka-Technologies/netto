...
pub fn main() -> Result<(), JsValue> {
    ...
    let document = Rc::new(
        window
            .document()
            .expect("Failed to get a reference to the document object"),
    );
    let table = Rc::new(
        document
            .query_selector("#metrics-table")?
            .expect("Failed to find main table element in document"),
    );
    let overhead_element = Rc::new(
        document
            .query_selector("#overhead")?
            .expect("Failed to find user-space overhead element in document"),
    );
    let power_element = Rc::new(
        document
            .query_selector("#power")?
            .expect("Failed to find power draw element in document"),
    );
    let procfs_table = Rc::new(
        document
            .query_selector("#procfs-table")?
            .expect("Failed to find procfs table in document"),
    );
    let svg_container = Rc::new(
        document
            .query_selector("#svg-container")?
            .expect("Failed to find svg container in document"),
    );
    ...
    let onmessage_callback =
        Closure::<dyn FnMut(_) -> _>::new(move |e: MessageEvent| -> Result<(), JsValue> {
                    ...
                    if let Ok(bytes) = res {
                        if let Ok(metrics) = MetricsWrapper::from_mp(&bytes) {
                            let perform_update = || -> Result<(), JsValue> {
                                ...
                                overhead_element.set_text_content(Some(&format!(
                                    "{: >6.02}%",
                                    metrics.user_space_overhead * 100.0
                                )));
                                power_element.set_text_content(Some(&if let Some(power) =
                                    metrics.net_power_w
                                {
                                    format!("{: >6.02}W", power)
                                } else {
                                ...
                                let mut svg_buf = String::new();
                                {
                                    let root = SVGBackend::with_string(
                                        &mut svg_buf,
                                        (CANVAS_SIZE_X as _, CANVAS_SIZE_Y as _),
                                    )
                                    .into_drawing_area()
                                    .apply_coord_spec(
                                        Cartesian2d::<RangedCoordf32, RangedCoordf32>::new(
                                            0.0..1.0f32,
                                            0.0..1.0f32,
