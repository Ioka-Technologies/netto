...
use crate::{
    bpf::ProgSkel,
    common::{
        self, event_types_EVENT_IO_WORKER, event_types_EVENT_MAX, event_types_EVENT_NET_RX_SOFTIRQ,
        event_types_EVENT_NET_TX_SOFTIRQ, event_types_EVENT_SOCK_RECVMSG,
...
impl TraceAnalyzer {
    ...
    pub fn new(
        ...
        let rapl = PowerCap::try_default().map(|rapl| rapl.intel_rapl).ok();
    ...
    fn run_interval(&mut self) -> anyhow::Result<()> {
        ...
        // Drain the stack traces array
        {
            ...
            (self.skel.bss().stack_traces_slot_off, num_traces_ref) = if slot_off > 0 {
                ...
                (
                    self.stack_traces_slot_size,
                    &mut self.skel.bss().stack_traces_count_slot_0,
                )
            };
            ...
            // Count symbols
            unsafe {
                for trace_ptr in (0..num_traces as usize).map(|trace_idx| {
                    self.stack_traces_ptr.add(
                        (slot_off + trace_idx) * 128, /* size of a single trace */
                    )
                }) {
                    // Get the cpuid
        ...
        let stats = self
            ...
            .ok_or(anyhow!(
                "Unexpected None returned for lookup into the \"per_cpu\" map"
            ))?;
        ...
        let total_cpu_frac = stats
            ...
            .sum::<f64>()
            / (self.prev_total_times.len() as f64);
        ...
        self.metrics_collector_addr.do_send(SubmitUpdate {
            net_power_w: delta_energy
                .map(|e| (e as f64) * total_cpu_frac / (delta_time.as_secs_f64() * 1_000_000.0)),
